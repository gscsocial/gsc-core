...
public class AssetIssueOperatorTest {
  ...
  public void createCapsule() {
    ...
    AccountWrapper ownerSecondCapsule =
        new AccountWrapper(
  ...
  public void rightAssetIssue() {
    AssetIssueOperator actuator = new AssetIssueOperator(getContract(), dbManager);
  ...
  public void repeatAssetIssue() {
    ...
    Any contract = Any.pack(
        Contract.AssetIssueContract.newBuilder()
            ...
            .setUrl(ByteString.copyFromUtf8(URL))
            .build());
    AssetIssueOperator actuator = new AssetIssueOperator(contract, dbManager); // second asset
    TransactionResultWrapper ret = new TransactionResultWrapper();
    long blackholeBalance = dbManager.getAccountStore().getBlackhole().getBalance();
    try {
      ...
      dbManager
          ...
          .put(
              ...
              new AssetIssueWrapper(getContract().unpack(Contract.AssetIssueContract.class)));
      // execute second assetissue
      actuator.validate();
      actuator.execute(ret);
      Assert.assertEquals(ret.getInstance().getRet(), code.SUCESS);
      AccountWrapper ownerSecond =
      ...
      Assert.assertNotNull(AssetIssueWrapper);
      String dbKey = new String(AssetIssueWrapper.createDbKey());
      Assert.assertEquals(dbKey, nameKey);
      Assert.assertEquals(AssetIssueWrapper.getOwnerAddress(),
          ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS_SECOND)));
  ...
  public void negativeTotalSupplyTest() {
    ...
    AssetIssueOperator actuator = new AssetIssueOperator(contract, dbManager);
    TransactionResultWrapper ret = new TransactionResultWrapper();
    long blackholeBalance = dbManager.getAccountStore().getBlackhole().getBalance();
    try {
      actuator.validate();
  ...
  public void assetNameTest() {
    ...
    actuator = new AssetIssueOperator(contract, dbManager);
