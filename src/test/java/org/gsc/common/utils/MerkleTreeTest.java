...
public class MerkleTreeTest {
    ...
    public void test0HashNumThrowIndexOutOfBoundsException() {
    ...
    public void test2HashNum() {
        ...
        MerkleTree.Leaf right = root.getRight();
    ...
    private static void pareTree(MerkleTree.Leaf head, List<Sha256Hash> hashList, int maxRank, int curBank,
                                 int number) {
        MerkleTree.Leaf left = head.getLeft();
        MerkleTree.Leaf right = head.getRight();
        if (curBank < maxRank) {
            curBank++;
            number = number << 1;
