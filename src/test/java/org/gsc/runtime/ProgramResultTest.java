...
public class ProgramResultTest {
  ...
  /**
   * pragma confirmed ^0.4.8; contract B{ address public calledAddress; constructor (address d)
  ...
  public void uniqueInternalTransactionHashTest()
    ...
    /* =================================== CALL create() =================================== */
    byte[] triggerData1 = GVMTestUtils.parseAbi("create()", "");
  ...
  public void successAndFailResultTest()
    ...
    // ======================================= Test Success =======================================
    byte[] triggerData1 = GVMTestUtils.parseAbi("transfer(address,bool)",
        params);
    Transaction trx1 = GVMTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), aContract,
            triggerData1, 0, 100000000);
    TransactionTrace traceSuccess = GVMTestUtils
    ...
    byte[] triggerData2 = GVMTestUtils.parseAbi("transfer(address,bool)",
        params);
    Transaction trx2 = GVMTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), aContract,
            triggerData2, 0, 100000000);
    TransactionTrace traceFailed = GVMTestUtils
        .processTransactionAndReturnTrace(trx2, deposit, null);
    runtime = traceFailed.getRuntime();
    byte[] bContract2 = Wallet
        .generateContractAddress(new TransactionWrapper(trx2).getTransactionId().getBytes(), 0);
  ...
  public void suicideResultTest()
    ...
    // ======================================= Test Suicide =======================================
    byte[] triggerData1 = GVMTestUtils.parseAbi("suicide(address)",
        params);
    Transaction trx = GVMTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), suicideContract,
            triggerData1, 0, 100000000);
    TransactionTrace trace = GVMTestUtils.processTransactionAndReturnTrace(trx, deposit, null);
  ...
  public void checkTransactionInfo(TransactionTrace trace, Transaction trx, BlockWrapper block,
      List<InternalTransaction> internalTransactionsList) {
    TransactionInfoWrapper trxInfoWrapper = TransactionInfoWrapper
        .buildInstance(new TransactionWrapper(trx), null, trace);
    List<Protocol.InternalTransaction> internalTransactionListFromProtocol = trxInfoWrapper
