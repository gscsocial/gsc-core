...
mapping(grcToken => uint256) tokenCnt;
...
function getTokenBalanceConstant(address addr, grcToken tokenId) constant returns(uint256 r) {
assembly{
r := tokenbalance(tokenId, addr)
}
}

function getTokenBalance(address addr, grcToken tokenId) returns(uint256 r) {
assembly{
r := tokenbalance(tokenId, addr)
}
}

function transferTokenInAssembly(address addr, grcToken tokenId, uint256 tokenValue) payable {
...
function grcTokenInMap(grcToken tokenId, uint256 tokenValue) returns(uint256 r) {
tokenCnt[tokenId] += tokenValue;
r = tokenCnt[tokenId];
}

function cntTokenTokenInMap(grcToken tokenId1, grcToken tokenId2, uint256 tokenValue) returns(grcToken r) {
