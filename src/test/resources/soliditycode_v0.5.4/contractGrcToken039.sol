 ...
 * 6. caller调用proxy中不存在的trans(uint256,address,grcToken)函数，注意这时grcToken是无意义的，但也带上tokenid。address是任意另外某账户的地址
 * 7. 可以看到目标地址trx增长5，caller账户trx减少5
 * 8. caller调用proxy中upgradeTo函数，传入B的地址
 * 9. caller调用proxy中不存在的trans(uint256,address,grcToken)函数。
...
contract A {
    function trans(uint256 amount, address payable toAddress, grcToken id) payable public {
