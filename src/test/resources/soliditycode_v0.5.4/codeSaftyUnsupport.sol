...
function() payable external{}
mapping(address => mapping(grcToken => uint256)) sender_tokens;

function trigger1(address payable addr, grcToken tokenInputId) payable public {
...
function trigger6(address payable addr, grcToken tokenId) payable public {
address(SubC(addr)).call.value(1000)(abi.encodeWithSignature("transferToken(uint256, grcToken)", 10, tokenId));
}
