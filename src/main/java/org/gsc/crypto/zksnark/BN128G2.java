...
public class BN128G2 extends BN128Fp2 {
    ...
    BN128G2(BN128<org.gsc.crypto.zksnark.Fp2> p) {
        super(p.x, p.y, p.z);
    }

    BN128G2(org.gsc.crypto.zksnark.Fp2 x, org.gsc.crypto.zksnark.Fp2 y, org.gsc.crypto.zksnark.Fp2 z) {
        super(x, y, z);
    }
    ...
    public static BN128G2 create(byte[] a, byte[] b, byte[] c, byte[] d) {
        ...
        BN128<org.gsc.crypto.zksnark.Fp2> p = BN128Fp2.create(a, b, c, d);
    ...
    private static boolean isGroupMember(BN128<org.gsc.crypto.zksnark.Fp2> p) {
        BN128<org.gsc.crypto.zksnark.Fp2> left = p.mul(FR_NEG_ONE).add(p);
    ...
    BN128G2 mulByP() {
        ...
        org.gsc.crypto.zksnark.Fp2 rx = TWIST_MUL_BY_P_X.mul(x.frobeniusMap(1));
        org.gsc.crypto.zksnark.Fp2 ry = TWIST_MUL_BY_P_Y.mul(y.frobeniusMap(1));
        org.gsc.crypto.zksnark.Fp2 rz = z.frobeniusMap(1);
