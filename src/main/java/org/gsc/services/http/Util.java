...
public class Util {
    ...
    public static JSONObject printBlockToJSON(Block block, boolean selfType) {
        ...
        String blockID = ByteArray.toHexString(blockWrapper.getBlockId().getBytes());
        ...
        if (!blockWrapper.getTransactions().isEmpty()) {
            jsonObject.put("transactions", printTransactionListToJSON(blockWrapper.getTransactions(),
                    selfType));
        }
        return jsonObject;
    }
    ...
    public static JSONArray printTransactionListToJSON(List<TransactionWrapper> list,
                                                       boolean selfType) {
        JSONArray transactions = new JSONArray();
        list.stream().forEach(transactionWrapper -> {
            transactions.add(printTransactionToJSON(transactionWrapper.getInstance(), selfType));
        });
        return transactions;
    }
            ...
            try {
                ...
                switch (contract.getType()) {
                    ...
                    case UpdateCpuLimitContract:
                        UpdateCpuLimitContract updateCpuLimitContract = contractParameter
                                .unpack(UpdateCpuLimitContract.class);
                        contractJson = JSONObject
                                .parseObject(JsonFormat.printToString(updateCpuLimitContract, selfType));
                        break;
            ...
            try {
                ...
                switch (contractType) {
                    ...
                    case "ExchangeWithdrawContract":
                        ...
                        break;
                    case "UpdateCpuLimitContract":
                        UpdateCpuLimitContract.Builder updateCpuLimitContractBuilder =
                                UpdateCpuLimitContract.newBuilder();
                        JsonFormat.merge(parameter.getJSONObject(VALUE).toJSONString(),
                                updateCpuLimitContractBuilder, selfType);
                        any = Any.pack(updateCpuLimitContractBuilder.build());
                        break;
                    case "AccountPermissionUpdateContract":
