...
import static org.gsc.common.utils.BIUtil.addSafely;
import static org.gsc.common.utils.BIUtil.isLessThan;
import static org.gsc.common.utils.BIUtil.isZero;
import static org.gsc.common.utils.ByteUtil.EMPTY_BYTE_ARRAY;
import static org.gsc.common.utils.ByteUtil.bytesToBigInteger;
import static org.gsc.common.utils.ByteUtil.numberOfLeadingZeros;
import static org.gsc.common.utils.ByteUtil.parseBytes;
import static org.gsc.common.utils.ByteUtil.parseWord;
import static org.gsc.common.utils.ByteUtil.stripLeadingZeroes;
...
import org.gsc.common.runtime.vm.program.ProgramResult;
import org.gsc.common.storage.Deposit;
import org.gsc.common.utils.BIUtil;
...
//    }
  ...
  public static class VoteWitnessNative extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      voteBuilder.setVoteAddress(ByteString.copyFrom(convertTogscAddress(witnessAddress20)));
      voteBuilder.setVoteCount(count);
      builder.addVotes(voteBuilder.build());
      VoteWitnessContract contract = builder.build();

      final List<Operator> operatorList = OperatorFactory
          .createActuator(new TransactionWrapper(contract), getDeposit().getDbManager());
      try {
...
//
//      final List<Operator> actuatorList = OperatorFactory
  ...
  public static class TransferAssetNative extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      builder.setToAddress(
          ByteString.copyFrom(convertTogscAddress(new DataWord(toAddress).getLast20Bytes())));
  ...
  public static class GetTransferAssetNative extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      long assetBalance = this.getDeposit().getDbManager().getAccountStore().
          get(convertTogscAddress(new DataWord(targetAddress).getLast20Bytes())).
