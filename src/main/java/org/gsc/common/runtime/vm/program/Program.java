...
import static org.gsc.common.runtime.utils.MUtil.transfer;
import static org.gsc.common.utils.BIUtil.isPositive;
import static org.gsc.common.utils.BIUtil.toBI;
...
import org.gsc.common.runtime.config.SystemProperties;
import org.gsc.common.runtime.vm.DataWord;
import org.gsc.common.runtime.vm.MessageCall;
import org.gsc.common.runtime.vm.OpCode;
import org.gsc.common.runtime.vm.PrecompiledContracts;
import org.gsc.common.runtime.vm.VM;
import org.gsc.common.runtime.vm.program.invoke.ProgramInvoke;
import org.gsc.common.runtime.vm.program.invoke.ProgramInvokeFactory;
import org.gsc.common.runtime.vm.program.invoke.ProgramInvokeFactoryImpl;
import org.gsc.common.runtime.vm.program.listener.CompositeProgramListener;
import org.gsc.common.runtime.vm.program.listener.ProgramListenerAware;
import org.gsc.common.runtime.vm.program.listener.ProgramStorageChangeListener;
import org.gsc.common.runtime.vm.trace.ProgramTrace;
import org.gsc.common.runtime.vm.trace.ProgramTraceListener;
import org.gsc.common.storage.Deposit;
import org.gsc.common.utils.ByteArraySet;
...
public class Program {
  ...
  public Program(byte[] ops, ProgramInvoke programInvoke, InternalTransaction transaction,
      SystemProperties config, BlockWrapper blockCap) {
    this(null, ops, programInvoke, transaction, config, blockCap);
  }

  public Program(byte[] codeHash, byte[] ops, ProgramInvoke programInvoke,
      InternalTransaction transaction, SystemProperties config, BlockWrapper blockCap) {
    this.config = config;
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = convertTogscAddress(getOwnerAddress().getLast20Bytes());
    byte[] obtainer = convertTogscAddress(obtainerAddress.getLast20Bytes());
  ...
  public void createContract(DataWord value, DataWord memStart, DataWord memSize) {
    ...
    byte[] senderAddress = convertTogscAddress(this.getOwnerAddress().getLast20Bytes());
...
//    ECKey ecKey = ECKey.fromPrivate(privKey);
    ...
    AccountWrapper existingAddr = getContractState().getAccount(newAddress);
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isInfoEnabled()) {
        logger.info("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(convertTogscAddress(getOwnerAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = convertTogscAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = convertTogscAddress(getOwnerAddress().getLast20Bytes());
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(convertTogscAddress(getOwnerAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit().getCode(convertTogscAddress(address.getLast20Bytes()));
  ...
  public DataWord getBlockHash(int index) {
    ...
    if (index < this.getNumber().longValue()
      ...
      List<BlockWrapper> blocks = this.invoke.getBlockStore().getBlockByLatestNum(index);

      if (CollectionUtils.isNotEmpty(blocks)) {
        BlockWrapper blockWrapper = blocks.get(0);
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState().getBalance(convertTogscAddress(address.getLast20Bytes()));
  ...
  public DataWord storageLoad(DataWord key) {
    DataWord ret = getContractState()
        .getStorageValue(convertTogscAddress(getOwnerAddress().getLast20Bytes()), key.clone());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(convertTogscAddress(msg.getType().callIsDelegate() ?
