...
import static org.gsc.crypto.Hash.EMPTY_TRIE_HASH;
import static org.gsc.utils.ByteUtil.EMPTY_BYTE_ARRAY;
import static org.gsc.utils.ByteUtil.toHexString;
import static org.gsc.core.wrapper.utils.RLP.EMPTY_ELEMENT_RLP;
import static org.gsc.core.wrapper.utils.RLP.encodeElement;
import static org.gsc.core.wrapper.utils.RLP.encodeList;
...
import org.gsc.core.wrapper.utils.FastByteComparisons;
import org.gsc.core.wrapper.utils.RLP;
import org.gsc.db.db2.common.ConcurrentHashDB;
...
public class TrieImpl implements Trie<byte[]> {
    ...
    private DB<byte[], BytesWrapper> cache;
    private Node root;
    ...
    public TrieImpl(DB<byte[], BytesWrapper> cache) {
        this(cache, null);
    }

    public TrieImpl(DB<byte[], BytesWrapper> cache, byte[] root) {
        this.cache = cache;
        setRoot(root);
    }
    ...
    public DB<byte[], BytesWrapper> getCache() {
        return cache;
    }

    private byte[] getHash(byte[] hash) {
        BytesWrapper bytesWrapper = cache.get(hash);
        return bytesWrapper == null ? null : bytesWrapper.getData();
    }

    private void addHash(byte[] hash, byte[] ret) {
        cache.put(hash, new BytesWrapper(ret));
    }
